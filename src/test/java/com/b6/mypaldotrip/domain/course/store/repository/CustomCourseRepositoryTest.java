    ...
    class 코스목록동적조회 {
        ...
        void 동적조회2() {
            ...
            ReflectionTestUtils.setField(
                    user2,
                    "followingList",
                    List.of(FollowingEntity.builder().user(user2).followingUser(user1).build()));
            // when
            Page<CourseEntity> res =
                    customCourseRepository.getCourseListByDynamicConditions(
                            pageable,
                            ...
                            req.filterByFollowing());
            // then
            assertThat(user2.getFollowingList().get(0).getFollowingUser())
                    .isEqualTo(res.getContent().get(0).getUserEntity());
            assertThat(res.getTotalElements()).isEqualTo(1);
        }
    }
